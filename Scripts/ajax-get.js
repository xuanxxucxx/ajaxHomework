...
document.querySelector('.btn').addEventListener('click', Getget, false);


function Getget() {
    //.open(格式, 要讀取的網址, 同步與非同步);
    //格式：get(讀取)、post(傳送資料到伺服器)
    xhr.open('get', 'https://raw.githubusercontent.com/xuanxxucxx/ajaxHomework/master/JSON/data.json', true);
